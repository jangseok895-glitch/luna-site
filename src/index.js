...
C. 랭킹전 / 경기 종료 순위표
- 본인 행은 아래 세 가지 선택 표식을 반드시 교차 확인하여 판별합니다.
- 1순위 단서: 맨 오른쪽에 다른 선수들에게 보이는 엄지척(좋아요) 아이콘이 본인 행에는 없습니다.
- 2순위 단서: 평균속도 숫자 바로 왼쪽 배경에 방패·육각형·광택처럼 보이는 선택 효과가 있습니다.
- 3순위 단서: 등수 숫자 바로 왼쪽과 행 양쪽 끝에 굵고 진한 아쿠아색 세로 강조선이 있습니다.
- 엄지척이 없는 행을 먼저 후보로 잡고, 같은 행에 방패 효과 또는 아쿠아 세로선이 있는지 확인하세요.
- 세 단서 중 최소 두 가지가 같은 행을 가리킬 때만 그 행을 본인 행으로 확정하세요.
- 세 단서가 서로 다른 행을 가리키면 임의로 1등이나 가장 밝은 행을 선택하지 마세요.
- 행 전체 밝기, 노란 기록, 노란 닉네임, 노란 평균속도, 1등 표시는 본인 판별 근거가 아닙니다.
- 왼쪽의 승리/패배 글자 뒤쪽 아쿠아 배경은 해당 행의 왼쪽 세로선과 연결될 때만 보조 단서로 사용하세요.
...
- 세 번째 이미지는 같은 선수 행을 세로로 맞춘 복합 안내 이미지입니다: 왼쪽 아쿠아 세로선 / 평균속도 왼쪽 방패 효과 / 오른쪽 엄지척 존재 여부.
- 사전 판별된 selectedRank를 우선하되, selectedRank가 3단서 교차 확인과 명백히 충돌하면 기록을 반환하지 말고 warning에 적으세요.
...
본인 등수 판별은 반드시 아래 세 단서를 같은 행에서 교차 확인하세요.

1순위: 맨 오른쪽 엄지척(좋아요) 아이콘 부재
- 다른 선수 행에는 맨 오른쪽에 엄지척 아이콘이 보이지만 본인 행에는 엄지척 아이콘이 없습니다.
- 친구, 공유, 사람 모양 등 다른 아이콘과 엄지척을 혼동하지 마세요.
- 이미지 오른쪽이 잘렸다면 이 단서는 사용할 수 없는 것으로 처리하세요.

2순위: 평균속도 바로 왼쪽의 방패형 선택 효과
- 평균속도 숫자 바로 왼쪽 배경에 방패·육각형·광택·반투명 문양처럼 보이는 선택 효과가 있는 행이 본인 후보입니다.
- 카트 그림이나 장식 아이콘을 방패 효과로 착각하지 마세요.

3순위: 등수 숫자 바로 왼쪽 및 행 양쪽 끝의 아쿠아 세로선
- 선택된 행은 등수 숫자 바로 왼쪽에 다른 행보다 굵고 진한 아쿠아색 세로선이 있습니다.
- 행 오른쪽 끝에도 같은 높이의 아쿠아 세로 경계가 보일 수 있습니다.
- 승리/패배 글자 뒤쪽의 진한 아쿠아 영역이 등수 왼쪽 세로선과 이어지면 같은 행의 보조 단서입니다.

판정 규칙
- 세 단서 중 최소 두 가지가 같은 행을 가리킬 때 그 등수를 selectedRank로 반환하세요.
- 엄지척이 없는 행 + 나머지 단서 하나가 일치하면 매우 강한 증거입니다.
- 엄지척 영역이 화면 밖이거나 잘렸다면 방패 효과 + 아쿠아 세로선 두 가지가 일치해야 합니다.
- 노란 기록, 노란 닉네임, 노란 속도, 1등 여부, 행 전체 밝기는 모두 무시하세요.
- 단서가 충돌하거나 한 가지 단서만 보이면 selectedRank는 null로 반환하세요.
...
- 그 외 unknown
  ...
  if (image.aquaGuideBase64) {
    content.push(
      {
        type: "input_text",
        text: "아래 복합 확대 이미지는 같은 높이의 선수 행을 세 패널로 보여줍니다. ① 등수 왼쪽 아쿠아 세로선 ② 평균속도 왼쪽 방패형 선택 효과 ③ 오른쪽 엄지척 아이콘 존재/부재를 같은 행끼리 교차 확인하세요. 엄지척이 없고 다른 단서도 일치하는 행을 선택하세요.",
      },
      {
        type: "input_image",
...
async function callVisionForSingleImage({
  ...
  if (image.aquaGuideBase64) {
    inputContent.push(
      {
        type: "input_text",
        text: "아래 이미지는 랭전 3단서 복합 안내 이미지입니다. 같은 높이의 행에 대해 ① 등수 왼쪽 아쿠아 세로선 ② 평균속도 왼쪽 방패형 효과 ③ 오른쪽 엄지척 아이콘 부재를 교차 확인하세요. 노란 글씨와 1등 여부는 무시하세요.",
      },
      {
        type: "input_image",
        image_url: `data:${image.aquaGuideMimeType || "image/jpeg"};base64,${image.aquaGuideBase64}`,
...
async function analyzeImages(request, env) {
  ...
  for (const image of acceptedImages) {
    ...
    if (
      ...
      warnings.push(`${image.name}: 엄지척 부재·평균속도 왼쪽 방패 효과·등수 왼쪽 아쿠아 세로선 중 최소 두 단서가 일치하지 않아 랭전 기록을 제외했습니다.`);
      continue;
    }
