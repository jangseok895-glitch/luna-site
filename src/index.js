...
- 행 전체 밝기만으로 판단하지 말고, 행 왼쪽 끝의 조금 두껍고 밝은 아쿠아색 세로 테두리를 최우선으로 찾습니다.
- 그 세로선이 위·아래 아쿠아색 경계와 연결되어 감싸는 정확한 행을 본인 행으로 선택합니다.
- 노란색 기록, 노란색 닉네임, 평균속도 색상은 본인 판별 기준에서 제외합니다.
...
- 두 번째 이미지는 랭킹전일 때 본인 행을 찾는 데만 쓰는 아쿠아 테두리 Crop 후보입니다.
- 원본이 여러 기록 카드 화면이라면 Crop 이미지는 완전히 무시하고 원본의 모든 카드를 추출하세요.
- Crop 때문에 원본의 다른 기록들을 누락해서는 안 됩니다.
` : `
