...
C. 랭킹전 / 경기 종료 순위표
- 본인 행 판별은 반드시 아래 우선순위를 따르세요.

[절대 1순위: 맨 오른쪽 엄지척 아이콘 부재]
- 다른 선수 행의 맨 오른쪽에는 흐린 엄지척(좋아요) 아이콘이 있습니다.
- 본인 행에는 이 엄지척 아이콘이 없습니다.
- 오른쪽 영역이 정상적으로 보이고 엄지척이 없는 행이 정확히 하나라면 그 행을 본인 행으로 최우선 확정하세요.
- 엄지척은 매우 희미할 수 있으므로 각 행의 맨 오른쪽을 확대해 한 줄씩 비교하세요.
- 친구 추가, 사람 모양, 공유 화살표, 말풍선 등은 엄지척이 아닙니다.
- 엄지척이 없는 행을 찾았으면 노란 기록, 순위, 밝은 배경 때문에 다른 행으로 바꾸지 마세요.

[2순위: 평균속도 왼쪽 방패형 선택 효과]
- 엄지척 부재를 명확히 구별하지 못했을 때만 사용하세요.
- 평균속도 숫자 바로 왼쪽 배경에 방패·육각형·광택·반투명 문양처럼 보이는 효과가 있는 행이 본인 후보입니다.
- 엄지척 없는 행과 방패 효과 행이 같으면 본인 행으로 매우 강하게 확정하세요.

[3순위: 아쿠아 테두리와 양쪽 세로선]
- 앞의 두 기준으로도 확정하지 못했을 때만 최후 보조 기준으로 사용하세요.
- 등수 숫자 바로 왼쪽과 행 양쪽 끝의 굵고 진한 아쿠아색 세로선, 위아래 아쿠아 테두리를 확인하세요.
- 촬영 각도와 밝기 때문에 아쿠아색은 오인식이 많으므로 단독으로 본인 행을 확정하지 마세요.
...
- 노란 평균속도
- 1등 여부
위 조건은 본인 판별과 관련이 없습니다.

- 사전 판별 단계에서 선택된 등수는 ${selectedRank === null ? "확정되지 않음" : `${selectedRank}등`}입니다.
- 화면 유형 판별 결과는 ${screenType}입니다.
${selectedRank !== null ? `- 반드시 ${selectedRank}등 행의 완주 기록만 읽으세요. 다른 행 기록은 반환하지 마세요.` : "- 선택 등수가 확정되지 않았다면 임의로 1등이나 2등을 선택하지 말고 기록을 반환하지 마세요."}
...
- 세 번째 이미지는 같은 선수 행을 세로로 맞춘 복합 안내 이미지입니다: ① 오른쪽 엄지척 존재/부재 ② 평균속도 왼쪽 방패 효과 ③ 왼쪽 아쿠아 세로선. 반드시 이 우선순위대로 판단하세요.
- 사전 판별된 selectedRank를 우선하되, selectedRank가 오른쪽 엄지척 부재 판독과 명백히 충돌하면 엄지척 부재 판독을 우선하고 충돌 내용을 warning에 적으세요.
...
본인 판별 우선순위는 아래와 같으며 순서를 절대로 바꾸지 마세요.

절대 1순위: 맨 오른쪽 엄지척(좋아요) 아이콘이 없는 행
- 다른 선수들의 맨 오른쪽에는 흐리거나 반투명한 엄지척 아이콘이 있습니다.
- 본인 행에는 엄지척 아이콘이 없습니다.
- 엄지척은 흐릿할 수 있으므로 오른쪽 끝을 행별로 확대해서 비교하세요.
- 오른쪽 영역이 정상적으로 보이고 엄지척이 없는 행이 정확히 하나라면 즉시 그 등수를 selectedRank로 선택하세요.
- 엄지척이 없는 행을 찾은 뒤에는 노란 기록, 1등 여부, 밝은 배경 때문에 다른 행으로 변경하지 마세요.
- 친구 추가 아이콘, 사람 아이콘, 공유 화살표, 말풍선은 엄지척이 아닙니다.

2순위: 평균속도 바로 왼쪽의 방패형·광택형 선택 효과
- 엄지척 부재가 불명확할 때만 사용하세요.
- 평균속도 숫자 바로 왼쪽 배경에 방패, 육각형, 반투명 광택 같은 효과가 있는 행을 찾으세요.

3순위: 아쿠아 테두리와 양쪽 세로선
- 앞의 두 기준으로도 판단하지 못했을 때만 사용하세요.
- 등수 숫자 왼쪽과 행 양쪽 끝의 진한 아쿠아 세로선, 위아래 테두리를 확인하세요.
- 아쿠아색은 촬영 각도와 화면 밝기에 따라 오인식될 수 있으므로 단독으로 확정하지 마세요.

금지 기준
- 노란 기록, 노란 닉네임, 노란 속도, 1등 여부, 행 전체 밝기는 본인 판별 기준이 아닙니다.
- 절대로 기본값으로 1등이나 2등을 선택하지 마세요.

selectedRank 결정 규칙
- 엄지척이 없는 행이 정확히 하나이고 오른쪽 영역이 보이면 그 행을 선택합니다.
- 엄지척 판단이 불명확하면 방패 효과를 사용합니다.
- 아쿠아 테두리는 마지막 확인용입니다.
- 모든 기준이 불명확하면 selectedRank는 null입니다.
...
- 그 외 unknown
  ...
  if (image.aquaGuideBase64) {
    content.push(
      {
        type: "input_text",
        text: "아래 복합 확대 이미지는 같은 높이의 선수 행을 세 패널로 보여줍니다. ① 오른쪽 엄지척 존재/부재 ② 평균속도 왼쪽 방패형 선택 효과 ③ 등수 왼쪽 아쿠아 세로선 순서입니다. 오른쪽이 정상적으로 보이면 엄지척이 없는 유일한 행을 최우선 선택하세요.",
      },
      {
        type: "input_image",
...
async function callVisionForSingleImage({
  ...
  if (image.aquaGuideBase64) {
    inputContent.push(
      {
        type: "input_text",
        text: "아래 이미지는 랭전 우선순위 복합 안내 이미지입니다. ① 오른쪽 엄지척이 없는 행을 먼저 찾고 ② 평균속도 왼쪽 방패 효과로 확인하며 ③ 아쿠아 테두리는 마지막 보조로만 사용하세요. 노란 글씨와 1등 여부는 무시하세요.",
      },
      {
        type: "input_image",
        image_url: `data:${image.aquaGuideMimeType || "image/jpeg"};base64,${image.aquaGuideBase64}`,
...
async function analyzeImages(request, env) {
  ...
  for (const image of acceptedImages) {
    ...
    if (
      ...
      warnings.push(`${image.name}: 오른쪽 엄지척이 없는 본인 행을 확실히 찾지 못했고, 방패 효과로도 보완 판별하지 못해 랭전 기록을 제외했습니다.`);
      continue;
    }
